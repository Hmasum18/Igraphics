...
{
    for(int i=0;i<100;i++) ///if w or space is pressed
    {
...
void iDraw()
{
    ...
    pause_resume();
   // if(mx>=19&&mx<=19+127&&my>=19&&my<=19+58)
    //iRectangle(151,29,107,55);
  ///  iRectangle(36,27,73,73);
}
...
void iMouse(int button, int state, int mx, int my)
{
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
        ...
        {
            if(mx>=521&&mx<=521+290&&my>=380&&my<=380+72) ///click start game
            {
               level_cnt=1;
                //level_cnt=3;
               level1_initial();
             ...
             if(mx>=521&&mx<=521+290&&my>=280&&my<=280+72) ///click leader board;
             {
                 menu.leader_board=1;
             }

              ///Rectangle(151,29,107,55);
              if(mx>=151&&mx<=151+107&&my>=29&&my<=29+55) ///click about
             {
                 printf("1\n");
                 menu.credit=1;
             }

             if(menu.credit) ///click back in credit (18,33,127,57);
                if(mx>=18&&mx<=18+127&&my>=33&&my<=33+58)
             {
                 menu.credit=0;
             }

            if(mx>=521&&mx<=521+290&&my>=180&&my<=180+72) ///click instuction board;
             {
                 menu.instruction=1;
             }
             if(menu.instruction)
             if(mx>=19&&mx<=19+127&&my>=20&&my<=20+58) ///click back in instruction
             {
                 menu.instruction=0;
             }

             if(mx>=521&&mx<=521+287&&my>=80&&my<=80+72) ///click sound
             {
                 if(!sound_mode) sound_mode=1;
                 else sound_mode=0;
                 if(!sound_mode) PlaySound(0,0,0);
                 if(sound_mode) PlaySound(sound[level_cnt], NULL,SND_LOOP | SND_ASYNC);
             }
             if(menu.leader_board)
             {
                // iRectangle(14,24,127,58);
                 ...
                 {
                       if(pause_temp>0) ///if we want to see leader board during the level
                       {
                           level_cnt=pause_temp;
                           pause_temp=0; ///game is not paused
                           click_leader=0; ///shows the pause buttons again
                       }
                       menu.leader_board=0;
                      // level_cnt==0;
                     ///  if(sound_mode) PlaySound(sound[level_cnt], NULL,SND_LOOP | SND_ASYNC);
        ...
        if(gameover()||mission_accomplished)
        {
        ...
        if(pause)
        {
            if(mx>=305&&mx<=305+93&&my>=212&&my<=212+93) ///click resume
                     pause=0;
            if(mx>=405&&mx<=405+93&&my>=212&&my<=212+93) ///click home
            {
                pause_game();
                level_cnt=0;
            ...
            if(mx>=506&&mx<=506+93&&my>=212&&my<=212+93) ///click leaderboard;
            {
                pause_temp=level_cnt;
                level_cnt=0; ///for not showing the level;
                menu.leader_board=1;
                click_leader=1;
            }
            if(mx>=608&&mx<=608+93&&my>=212&&my<=212+93) ///click sound button
            {
                 if(!sound_mode) sound_mode=1;
                 else sound_mode=0;
                 if(!sound_mode) PlaySound(0,0,0);
...
{
    if(pause)
    if(key==27) ///press escape button to pause game
    {
        pause=0;
    }
...
void iSpecialKeyboard(unsigned char key)
{
    ...
    {iShowBMP2(scr_x-200,30,"menu\\home.bmp",170);
        if(man_idx==4) man_idx=3;
        if(man_x>=7)
            man_x-=7;
        man_idx--;
...
void arrowChange()
{
    ...
    if(level_cnt==0) ///for menu;
    {
        ...
        if(drag.game<=20&&drag.start>521) drag.start-=30;
        if(drag.start<=521&&drag.leader>521) drag.leader-=30;
        if(drag.leader<=521&&drag.instruction>521) drag.instruction-=30;
        if(drag.instruction<=521&&drag.sound>521) drag.sound-=30;
        //if(start>521) start-=20;
       // if(start==521&&leader>521) leader-=20;
    }
    if(nameinput)
    {
        if(drag.new_highest_score>=260) drag.new_highest_score-=50;
        if(drag.new_highest_score<=260&&drag.enter_name<=260) drag.enter_name+=35;
    }
}
void levelTimer()
{
...
void load_page()
{
    if(level_cnt<0) level_cnt++; ///for loading page-55
    ///for after game show
    if(gameover()&&after_over<2) after_over++;
    if(you_won(level_cnt)&&after_won<3) after_won++;
}

int main()
{
    sound_mode=1;
    drag.ball=970,drag.game=970,drag.start=970,drag.instruction=970,drag.leader=970,drag.sound=970,drag.new_highest_score=970,drag.enter_name=-448;
